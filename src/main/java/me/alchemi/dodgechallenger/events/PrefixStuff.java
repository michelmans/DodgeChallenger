	...
	public static void setRankPrefix(Player player, String rank) {
		...
		if (main.luckPermsEnabled) {
			...
			if (main.chatEnabled) {
				...
				if (!pref.contains(Messenger.formatString(rank))) player.setDisplayName(Messenger.formatString(rank + PersistentMeta.getMeta(player, PrefixMeta.class).asString()));
			}
		}
	}
